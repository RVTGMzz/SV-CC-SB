...
for frame in range(4):
    d=ImageDraw.Draw(totem); ox=frame*W
    if frame<3:
        d.rectangle((ox+7,38,ox+24,43), fill=(55,46,33,255)); d.rectangle((ox+4,43,ox+27,46), fill=(38,34,27,255))
        d.rectangle((ox+10,21,ox+21,39), fill=(80,63,40,255)); d.rectangle((ox+8,24,ox+23,34), fill=(97,75,43,255)); d.rectangle((ox+11,14,ox+20,24), fill=(61,82,45,255))
        d.polygon([(ox+8,23),(ox+2,18),(ox+4,28),(ox+10,31)], fill=(63,113,58,255)); d.polygon([(ox+23,23),(ox+29,18),(ox+27,28),(ox+21,31)], fill=(63,113,58,255))
        core=(127,232,103,255) if frame==0 else (201,210,91,255) if frame==1 else (239,151,76,255)
        d.polygon([(ox+16,15),(ox+22,22),(ox+20,30),(ox+16,34),(ox+12,30),(ox+10,22)], fill=core); d.rectangle((ox+14,20,ox+17,26), fill=(224,255,174,255)); d.rectangle((ox+9,35,ox+22,36), fill=(139,143,82,255))
        if frame>=1: d.line((ox+14,18,ox+18,24,ox+13,30), fill=(72,46,33,255), width=1)
        if frame>=2:
            d.line((ox+21,22,ox+17,29,ox+22,34), fill=(72,46,33,255), width=2); d.rectangle((ox+4,39,ox+8,42), fill=(97,75,43,255))
    else:
        d.rectangle((ox+6,42,ox+25,46), fill=(38,34,27,255)); d.rectangle((ox+10,37,ox+21,42), fill=(74,57,37,255))
...
def box(col,row,rect,color):
    d=ImageDraw.Draw(hub); x0=col*cell; y0=row*cell; d.rectangle((x0+rect[0],y0+rect[1],x0+rect[2],y0+rect[3]),fill=color)
def line(col,row,pts,color,w=1):
    d=ImageDraw.Draw(hub); x0=col*cell; y0=row*cell; d.line([(x0+x,y0+y) for x,y in pts],fill=color,width=w)
...
s=s2.replace('ArenaSeal=ON | Obelisks=4 | Motes=ON | IntroBars=ON', 'ArenaSeal=ON | DestructibleTotems={this.Boss.GetLivingTotemCount()}/4 | Barrier={this.Boss.GetTotemDamageReductionPercent()}% | Motes=ON | IntroBars=ON',1)
p.write_text(s,encoding='utf-8')
...
s=s.replace(anchor,decor_methods+anchor,1); p.write_text(s,encoding='utf-8')
...
if cmd_anchor in s: s=s.replace(cmd_anchor,cmd_anchor+'        helper.ConsoleCommands.Add("cardcha_boss1_totem_status", "Show destructible Verdant Seed Totem state.", (_, _) => this.Monitor.Log(this.VerdantGuardian.DescribeBalance(), LogLevel.Alert));\n',1)
p.write_text(s,encoding='utf-8')
...
add_i18n(ROOT/'i18n'/'default.json',{'boss.verdant.totem.broken':'A Verdant Seed Totem shattered. {{remaining}} remain.','boss.verdant.totem.all-broken':'VERDANT BARRIER BROKEN! The Guardian reels as the last seed totem collapses.','airship.lostfound.empty':'LOST & FOUND\\nNothing has been returned here right now.'})
add_i18n(ROOT/'i18n'/'vi.json',{'boss.verdant.totem.broken':'Một Verdant Seed Totem đã vỡ. Còn lại {{remaining}} trụ.','boss.verdant.totem.all-broken':'LỚP CỘNG HƯỞNG VERDANT ĐÃ VỠ! Guardian khựng lại khi trụ cuối cùng sụp xuống.','airship.lostfound.empty':'KHO ĐỒ THẤT LẠC\\nHiện chưa có vật phẩm nào được gửi về đây.'})

Path('handoff/ALPHA28_0668_TOTEM_AIRSHIP_HUB_CLEANUP.md').write_text(f'''# Alpha28 0668 - Verdant Totem + Airship/Hub Cleanup\n\nBuild: `{VERSION}`\nBranch: `cardcha-alpha28-0668-totem-airship-hub-cleanup`\nStatus: implementation candidate; in-game acceptance pending.\n\n## Verdant Seed Totems\n- Four former corner props are real targetable stationary support totems, 90 HP each.\n- Boss barrier 4/3/2/1/0 = 15/10/6/3/0% damage reduction.\n- >=2 surviving totems shorten Root/Vine cooldown 8%.\n- Last totem break interrupts the current attack and staggers Boss I 1.2s.\n- Totems never award Cardcha drops/materials.\n- Vanilla proxy art hidden by Harmony while proxies remain targetable to party AI.\n\n## Airship/Hub\n- Deck + Sky Dock get warm asymmetrical Stardew-style clutter.\n- Boarding gangway gets cargo, rope and lamps.\n- Lost & Found is a physical chest with no floating name/hint; interaction shows its name/status only when opened.\n- Region I rooms get a light edge-only flora/stone/stump pass without collision edits.\n\nAirship visual asset/hash is untouched. Hunt Run 7-10, 0660-0667, schema 19, Guardian Rabbit 10s and Boss Energy x1/3 are preserved.\n''',encoding='utf-8')
