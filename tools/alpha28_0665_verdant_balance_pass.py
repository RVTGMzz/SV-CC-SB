...
old_anchor = '''        // Colossus weight: normal attacks cannot shove the boss around. Only Cardcha-owned
...
'''
new_anchor = '''        // 0665 heavy-body tuning: allow a tiny hit reaction, but never cumulative wall-pinning.
        // HeavyAnchor only moves through Cardcha-owned motion (Charge/phase recenter).
        if (this.State != VerdantGuardianState.Charging && this.State != VerdantGuardianState.PhaseTransition)
            this.ApplyHeavyRecoil(boss);
'''
if old_anchor not in boss:
    raise RuntimeError('heavy anchor block missing')
boss = boss.replace(old_anchor, new_anchor, 1)
...
for old, new in repls.items():
    if old not in boss:
        raise RuntimeError(f'boss damage anchor missing: {old}')
    boss = boss.replace(old, new, 1)

# Adds: slightly less spongey than the 0663 visual prototype, but still meaningful distractions.
boss = boss.replace('add.MaxHealth = this.Phase switch { 1 => 38, 2 => 52, _ => 68 };',
                    'add.MaxHealth = this.Phase switch { 1 => 34, 2 => 46, _ => 58 };', 1)
boss = boss.replace('add.Speed = this.Phase switch { 1 => 4, 2 => 5, _ => 6 };',
                    'add.Speed = this.Phase switch { 1 => 4, 2 => 5, _ => 5 };', 1)
boss = boss.replace('add.MaxHealth = this.Phase switch { 1 => 55, 2 => 75, _ => 95 };',
                    'add.MaxHealth = this.Phase switch { 1 => 50, 2 => 68, _ => 84 };', 1)

old_cooldowns = '''    private static int CooldownMs(VerdantGuardianAttack attack, int phase) => attack switch
    {
...
'''
new_cooldowns = '''    private static int CooldownMs(VerdantGuardianAttack attack, int phase) => attack switch
    {
...
'''
if old_cooldowns not in boss:
    raise RuntimeError('cooldown block missing')
boss = boss.replace(old_cooldowns, new_cooldowns, 1)

helper_anchor = '    private static Point[] BuildRootTargets(Point center, int phase)\n'
if helper_anchor not in boss:
    raise RuntimeError('BuildRootTargets anchor missing')
...
'''
boss = boss.replace(helper_anchor, helpers + helper_anchor, 1)
...
'''
new_prefix = '''            bool isVerdantGuardian = __instance.modData.ContainsKey(VerdantGuardianBossService.BossMarkerKey);
            ...
            // 0665: the Guardian can visibly recoil a few pixels, but receives only 8% of the
            // final trajectory and is hard-capped. The boss service then recenters it toward
            // HeavyAnchor, so a full party cannot volleyball/pin it against the arena wall.
...
'''
if old_prefix not in patch:
    raise RuntimeError('MonsterDamagePatch Colossus block missing')
patch = patch.replace(old_prefix, new_prefix, 1)
patch_path.write_text(patch, encoding='utf-8')

# ---------------- Guardian Rabbit final Region I damage candidate ----------------
...
# ---------------- i18n: keep descriptions truthful ----------------
...
# ---------------- ModEntry command/version ----------------
mod_path = ROOT / 'ModEntry.cs'
mod = mod_path.read_text(encoding='utf-8')
command_anchor = '        helper.ConsoleCommands.Add("cardcha_boss1_polish_status", "Show Boss I arena/camera/sound polish diagnostics.", (_, _) => this.Monitor.Log(this.VerdantArenaPolish.Describe(), LogLevel.Alert));\n'
if command_anchor not in mod:
    raise RuntimeError('0664 polish command anchor missing')
mod = mod.replace(command_anchor, command_anchor +
...
mod = mod.replace('0.3.0-alpha.28.0.4.14.4.5.12.31 VERDANT ARENA CINEMATIC POLISH TEST',
                  '0.3.0-alpha.28.0.4.14.4.5.12.32 VERDANT REGION I BALANCE PASS TEST', 1)
mod_path.write_text(mod, encoding='utf-8')

# ---------------- handoff ----------------
handoff = Path('handoff/ALPHA28_0665_VERDANT_REGION1_BALANCE_PASS.md')
handoff.write_text(f'''# Alpha28 0665 - Verdant Region I Balance Pass\n\nBuild: `{VERSION}`\nBranch: `cardcha-alpha28-0665-verdant-balance-pass`\nStatus: balance candidate; in-game acceptance for 0660-0665 remains pending.\n\n## Verdant Guardian\n- HP: 1300 -> 1600. This is a +23% durability pass, not a sponge-doubling pass.\n- Damage now ramps by phase while Phase 1 keeps the old learning-floor values.\n  - Swipe: 10 / 12 / 14\n  - Root: 8 / 10 / 12\n  - Charge: 14 / 16 / 19\n  - Vine: 4 / 5 / 7\n  - Slam: 16 / 18 / 22\n- Attack cooldowns/decision gaps are slightly longer than the raw prototype so later phases are readable instead of chain-locking the player.\n- Telegraph durations and all 0660 animation timings are unchanged.\n\n## Heavy-body / knockback\n- Absolute 0-trajectory immunity is replaced by 8% final knockback with a 12 trajectory cap.\n- Boss displacement is clamped to an 18 px recoil envelope and lerps back to HeavyAnchor.\n- HeavyAnchor still moves only from Cardcha-owned charge/phase movement, so team hits cannot cumulatively pin the boss into a wall.\n\n## Summons\n- Briarling HP: 50 / 68 / 84; speed 2 / 3 / 3.\n- Leaf Wisp HP: 34 / 46 / 58; speed 4 / 5 / 5.\n- Max active adds remains 4 and summon composition/cap stays exactly as 0663.\n\n## Guardian Rabbit\n- Boss Form duration remains exactly 10 seconds.\n- Root Pulse interval remains 2 seconds; radius remains 176 px.\n- Root Pulse damage: 18 -> 14 per target.\n- Boss Energy cost stays 100 and gain scale stays x1/3.\n\n## Verdant Core\n- Trigger stays at 50% projected HP.\n- Duration stays 6 seconds; regen stays 2 HP/s.\n- Damage reduction: 35% -> 30%.\n- Cooldown: 24s -> 28s.\n\n## Debug\n- `cardcha_boss1_balance_status` prints Boss I, Verdant Core and Guardian Rabbit balance telemetry.\n\n## Locked systems preserved\nSave schema 19, 20/40/60/80 milestones, 76 active normal cards / 80 stored IDs, 0659 MiMi stair resolver, MiMi profile/CC continuity, Region I Hunt Run 4-of-6, 0660 visual/Colossus art, 0663 custom summon visuals, 0664 arena/camera/sound presentation, Airship route/upgrades, controller mapping and Forest gate remain unchanged.\n''', encoding='utf-8')
...
print(json.dumps({
    ...
    'bossHP': 1600,
    'damage': {
        'swipe': [10,12,14], 'root': [8,10,12], 'charge': [14,16,19],
        'vine': [4,5,7], 'slam': [16,18,22]
    },
